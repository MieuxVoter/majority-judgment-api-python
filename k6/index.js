...
export default async function (data) {
    ...
    if (token != undefined) {
        ...
        check(ballotsPayload, {
            'GET ballots returns status 200': (r) => r.status === 200,
        });
    }
    ...
    for (let i = 0; i < candidates.length; ++i) {
        votes.push({
            ...
            grade: grades[Math.floor(Math.random() * grades.length)].id,
        });
    }

    if (token != null) {
        const ballotsPayload = await http.put(`${url}/ballots`, {
            votes: votes
        }, {
            headers: {
                "Authorization":`Bearer ${token}`,
        ...
        const ballotsPayload = http.post(`${url}/ballots`, {
            votes: votes,
